<?php
if (!class_exists('ATQuimicosClientsCPT')) {
    class ATQuimicosClientsCPT
    {
        function __construct()
        {
            add_action('init', array($this, 'create_post_type'));
            add_action('init', array($this, 'create_sede_taxonomy'));
            add_filter('use_block_editor_for_post_type', array($this, 'disable_gutenberg_for_atquimicosclients'), 10, 2);
        }

        public function create_post_type()
        {
            register_post_type('atquimicosclients', [
                'label' => 'ATQuimicos clients',
                'description' => 'ATQuimicos clients Post Type',
                'labels' => array(
                    'name' => 'ATQ clients',
        ...
        public function create_sede_taxonomy()
        {
            $labels = array(
                'name'              => _x('Sedes', 'taxonomy general name', 'textdomain'),
                'singular_name'     => _x('Sede', 'taxonomy singular name', 'textdomain'),
                'search_items'      => __('Buscar Sedes', 'textdomain'),
                'all_items'         => __('Todas las Sedes', 'textdomain'),
                'parent_item'       => __('Sede Padre', 'textdomain'),
                'parent_item_colon' => __('Sede Padre:', 'textdomain'),
                'edit_item'         => __('Editar Sede', 'textdomain'),
                'update_item'       => __('Actualizar Sede', 'textdomain'),
                'add_new_item'      => __('Añadir Nueva Sede', 'textdomain'),
                'new_item_name'     => __('Nombre de la Nueva Sede', 'textdomain'),
                'menu_name'         => __('Sede', 'textdomain'),
            );
            ...
            register_taxonomy('sede', array('atquimicosclients'), $args);
        }
